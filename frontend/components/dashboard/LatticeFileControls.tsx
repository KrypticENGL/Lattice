...
function ImportDialog({
  ...
  return createPortal(
    // The same shell as the post composer, down to the class lists: a
    // plain scrim, a `.glass` panel carrying the `.dialog-in` keyframe,
    // and a `.glass-bar` title strip. Two modals in one app that are the
    // same object should be built the same way — the alternative is the
    // pair drifting every time one of them is touched.
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 sm:p-6"
      onClick={onCancel}
    >
      {/* `max-w-lg` where the composer takes `max-w-3xl` is the one
        * measurement deliberately not copied: this dialog is a preview and
        * two buttons, and the same frame at three times the width would be
        * mostly empty. Everything that makes it the *same kind of window* —
        * the material, the title strip, the scrolling body, the footer — is
        * shared. */}
      <div
        className="glass dialog-in my-auto flex w-full max-w-lg flex-col overflow-hidden rounded-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* `relative z-10` for the reason the simulator's panels need it:
          * `.glass-bar` opens a stacking context with `isolation`, and so
          * does anything below it that paints or contains — without this
          * the strip can end up under the body it is supposed to head. */}
        ...
        <div className="flex max-h-[70vh] flex-col overflow-y-auto">
          {error ? (
            <p className="p-5 font-serif text-[14px] leading-6 text-[var(--text-secondary)]">
              {error}
            </p>
          ) : (
            file && (
