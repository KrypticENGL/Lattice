 ...
 * The community feed.
 *
 * One column, and a narrow one. These are posts with a diagram and three
 * paragraphs of prose, so the constraint that matters is line length, not
 * how much of the screen gets filled — a two-up grid of these would give
 * every card a 30-word measure and halve the size of the drawing that is
...
export default function PostFeed() {
  ...
  const visible = useMemo(() => searchPosts(POSTS, query), [query]);
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="rail-pill glass-flat flex gap-2 rounded-full px-3 font-mono text-[10px] uppercase tracking-wider text-[var(--text-secondary)]">
          {/* While searching this counts what is on screen, not what
          ...
          {searching ? `of ${POSTS.length} posts` : "posts"}
        ...
        <label className="rail-pill glass-flat flex min-w-0 flex-1 gap-2 rounded-full px-3 text-[var(--text-secondary)] focus-within:border-[var(--accent-secondary)] sm:max-w-[18rem] sm:flex-none">
          <SearchIcon />
      ...
      {visible.length === 0 && (
        <p className="py-16 text-center font-mono text-[11px] uppercase leading-relaxed tracking-wider text-[var(--text-secondary)]">
